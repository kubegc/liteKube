...
func (workerRuntime *WorkerRuntime) LoadNetManager() error {
	...
	if certificate.NotExists(raw.RegisterOptions.CACert, raw.RegisterOptions.ClientCertFile, raw.RegisterOptions.ClientkeyFile) && certificate.NotExists(raw.JoinOptions.CACert, raw.JoinOptions.ClientCertFile, raw.JoinOptions.ClientkeyFile) && raw.NodeToken == "" {
		...
		workerRuntime.RuntimeAuthentication.NetWorkManagerClient = leaderAuth.NewNetworkManagerClient(workerRuntime.RuntimeAuthentication.CertDir, new.Token)
		if err := workerRuntime.RuntimeAuthentication.NetWorkManagerClient.GenerateOrSkip(new.RegisterOptions.Address, int(new.RegisterOptions.SecurePort)); err != nil {
			return err
		}
